...
function createUser($conn, $name, $email, $geboorte, $werkopleiding, $startdatum, $inschrijfdatum, $pwd) {
  ...
  header("location: ../registratie.php?error=succes");
  exit();
}
...
function activiteitStart($conn, $id, $startdatum) {
  ...
  $sql = "SELECT * FROM jongerenactiviteit WHERE jongerenId = '$id' AND activiteitStartdatum = '$startdatum';";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
...
//ACTIVITEIT AVAILIBLE
function activiteitStatus($conn, $id, $activiteit, $status) {

  $sql = "SELECT * FROM jongerenactiviteit WHERE jongerenId = '$id' AND activiteitId = '$activiteit' AND activiteitAfgerond = 'ja';";
  $stmt = mysqli_stmt_init($conn);
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    header("location: ../jongerenactiviteit.php?error=stmtfailed");
    exit();
  }

  mysqli_stmt_bind_param($stmt, "iss", $id, $activiteit, $status);
  mysqli_stmt_execute($stmt);
